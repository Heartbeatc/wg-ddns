...
工具会自动进入 **6 步配置向导**：按顺序填写运行位置、入口节点、出口节点、Cloudflare、域名和自动化设置。6 步填完后，wgstack 会自动执行预检：验证 Cloudflare、创建/更新需要的 DNS 记录、等待解析生效，并检查入口/出口 SSH。预检失败时会回到摘要页，你可以跳回对应步骤修改；点击「开始部署」前也会再次自动预检。
