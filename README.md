...
工具会自动进入 **6 步配置向导**：按顺序填写运行位置、入口节点、出口节点、Cloudflare、域名和自动化设置。6 步填完后，wgstack 会自动执行预检：验证 Cloudflare、创建/更新需要的 DNS 记录、确认 Cloudflare 记录已写入，并检查入口/出口 SSH。本地或运营商 DNS 缓存没刷新时只会提示，不会阻塞部署。预检失败时会回到摘要页，你可以跳回对应步骤修改；点击「开始部署」前也会再次自动预检。
...
6. **出口管理 DDNS** — 默认启用，只需填写出口 SSH 管理域名和检查间隔
7. **入口自动修复** — 默认启用，入口节点定时修复 IP / DNS 漂移，检查间隔可配置
...
**向导方式**：首次运行 `wgstack` 时，向导默认启用入口自动修复，不再要求手动选择 Y/N；检查间隔默认 60 秒，可直接输入其他秒数（最小 60）。
...
**向导方式**：首次运行 `wgstack` 时，向导默认启用出口管理 DDNS，只需要填写出口 SSH 管理域名和检查间隔。检查间隔默认 60 秒，可直接输入其他秒数（最小 60）。
